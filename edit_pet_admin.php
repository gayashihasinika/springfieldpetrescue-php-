...
$pet_sql = "SELECT * FROM pet WHERE pet_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update_sql = "UPDATE pet SET name = ?, pet_type = ?, age = ?, gender = ?, district = ?, city = ?, phone_number = ?, description = ?, is_adopted = ? WHERE pet_id = ?";
    $update_stmt = $connection->prepare($update_sql);
    $update_stmt->bind_param("ssisssssii", $name, $pet_type, $age, $gender, $district, $city, $phone_number, $description, $is_adopted, $pet_id);

    if ($update_stmt->execute()) {
        header("Location: admin_dashboard.php?status=success&message=Pet+was+updated+succesfully!");
        exit();
    } else {
...
<head>
    ...
    <title>PAWS - Admin Edit Pet</title>
...
<header>
    ...
    <form action="edit_pet_admin.php?pet_id=<?php echo $pet_id; ?>" method="POST">
